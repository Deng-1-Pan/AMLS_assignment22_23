...
basedir_test = "./Datasets/dataset_AMLS_22-23_test/cartoon_set_test"
...
def resize_feature(features_left_eye, features_right_eye):
    '''
        This is a function to resize the feature size according to average size
        of the given feature regions.

        return: a resized feature region
    '''
...
def extract_features_labels(Label):
    ...
    # Sort the file in order
    images_dir_sorted = os.listdir(images_dir)
    images_dir_sorted = sorted(images_dir_sorted, key=key_func)
        ...
        with tqdm(total=total_img, unit="img", desc="Loading " + Label + " Dataset") as pbar:
            for img_path in image_paths:
                ...
                if features is not None:
                    ...
                    all_labels[1].append(int(eye_color_labels[int(file_name)]))
                pbar.update(1)
