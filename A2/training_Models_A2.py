...
def Model_Select(train_data, model_name):
    ...
    if model_name == 'SVM':
        ...
        model = AdaBoostClassifier(algorithm='SAMME', 
                                   base_estimator=svm.LinearSVC(C = 0.01, dual=False, random_state=random_seed), 
...
def Model_Training_Testing_A2(train_data, train_labels, test_data, test_labels, model_name):
    ...
    if model_index != 3:
        ...
        best_model = GridSearchCV(model, {'learning_rate': list_para[3]}, cv = 10, scoring = 'accuracy')
        t1 = time.perf_counter()
        best_model.fit(test_data, test_labels)
        t2 = time.perf_counter()
        para_name = 'learning_rate'
        print(f'The best hyperparameter for {model_name} is {best_model.best_params_[para_name]} with average accuracy {best_model.best_score_}')
        print(f"Training Completed! Time used: {t2-t1:.2f}s")
    ...
    plt.figure(num=f'Confusion Matrix A2 {model_name}')
    plt.title(f'Confusion Matrix A2 {model_name}')
